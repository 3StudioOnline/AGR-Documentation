...
The `AGR Inventory Component` is a flexible and fully network-replicated inventory
solution that works in tandem with the `AGR Item Component`.
...
* `Item` <Step text="1"/> : Item that was updated.
* `Update Type` <Step text="2"/> : [Type of the update](/docs/inventory-module/types#item-update-type) that happened.
...
* `Item` <Step text="1"/> : Item in equipment slot that was updated.
* `Slot Action` <Step text="2"/> : [Type of the action](/docs/inventory-module/types#equipment-slot-action) that happened.
* `Slot` <Step text="3"/> : Equipment slot that was updated.
