...
The `AGR Item Component` allows to turn any actor into an item that works in
...
* `Visibility` <Step text="1"/> : If true item became visible, otherwise false.
* `Simulate Physics` <Step text="2"/> : If true item started simulating physics,
otherwise false.
...
* `Inventory Component` <Step text="1"/> : Inventory component of an actor that
equipped this item.
* `Equipment Slot` <Step text="2"/> : Slot into which one the item was equipped.
...
* `Equipment Slot` <Step text="2"/> : Slot from which one the item was unequipped.
...
* `Inventory Component` <Step text="1"/> : Inventory component of an actor that
used this item.
* `Action Type` <Step text="2"/> : [Type of action](/docs/inventory-module/types#item-update-type)
that was called on this item.
...
* `Inventory Component` <Step text="1"/> : Inventory component of an actor that
dropped this item.
* `Drop Location` <Step text="2"/> : Items location where it was dropped.
