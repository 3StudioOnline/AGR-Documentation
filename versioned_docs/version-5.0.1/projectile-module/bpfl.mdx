...
`Get Projectile Launcher Component` gets the AGR Projectile Launcher component
from an actor.
...
the location of the impact point under the cursor, `Look at Rotation`
<Step text="6"/> the rotation needed to look at the impact point of a detected
object the cursor is aiming at and `Hit Result` <Step text="7"/> the hit result
of the line trace.
