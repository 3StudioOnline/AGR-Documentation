...
* `Class` <Step text="1"/> : Class of the items to filter by.
...
* `Gameplay Tag` <Step text="1"/> : Gameplay tag of the items to filter by.
...
* `Name` <Step text="1"/> : Name of the items to filter by.
* `Case Sensetive` <Step text="2"/> : If true, the text comparison of names will be case-sensitive.
...
* `Tags` <Step text="1"/> : Tags of the items to filter by.
* `Match All` <Step text="2"/> : If true will return only items that match all of the specified tags.
