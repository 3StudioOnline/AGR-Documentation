...
it will perform firing the weapon according to the selected fire mode.
...
Calls the `OnBurstInterrupted` event dispatcher when a burst fire action was
interrupted.
...
`GetSafeProjectileSpawnLocation()` and `CalculateFireSpread()` respectively.

Calls the `OnProjectileSpawned` event dispatcher after successfully spawning
the projectile.
...
* `Fire Mode` <Step text="1"/> : The new fire mode.
